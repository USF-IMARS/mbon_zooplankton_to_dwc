...
taxa_files <-
    ...
    mutate(
        ...
        cruise_id = case_when(
            ...
            str_detect(cruise_id, "WS18284") ~ "WS18218",
            str_detect(cruise_id, "WS8218") ~ "WS18218",
...
```{r load-data}
cid <- meta_df %$% unique(cruise_id)
cid1 <- taxa_files %$% unique(cruise_id)
cid[cid %in% cid1]
cid1[cid1 %in% cid]
cid[!(cid %in% cid1)]
cid1[!(cid1 %in% cid)]
...
taxa_matched_merg %>%
    # filter(cruise_id == "WS19210") %>%
    # distinct(site, mesh, .keep_all = TRUE)
    filter(is.na(flowmeter_in)) %>%
    full_join(filter(meta_df, cruise_id == "WS19210"), 
              by = c("cruise_id", 
                     "site" = "locationID", 
                     "mesh" = "mesh_size_um")) %>%
    select(1:3, mesh, file.y:last_col())

taxa_files %>%
    filter(str_detect(cruise_id, "WS2000")) %>%
    select(cruise_id, site, mesh) %>%
    distinct() %>%
    arrange(site, mesh)


taxa_matched_merg %>% 
    distinct(cruise_id, site, mesh) %>%
    arrange(cruise_id, site)
...
# here("data", "raw") %>%
    ...
    tibble(files = .) %>%
    mutate(
        base = fs::path_ext_remove(files),
        base = basename(base),
        # cruise = str_replace(base, "(.*)_(.*)_(.*)", "\\1"),
        
    ) #%>%
    filter(!str_detect(cruise, "~")) %>%
    arrange(cruise)
...
taxa_matched_merg <-
    ...
    mutate(
        ...
        datasetName         = "MBON Florida Keys National Marine Sanctuary Zooplankton Net Tows",
...
event <-
    ...
    dplyr::transmute(
        ...
        eventDate          = format(date_time, "%Y-%m-%dT%H:%m:%S%z"),
