...
i = 1
!fs::file_exists(paste0(new_dir, "/",
                   basename(test[i]), "/",
                   basename(fs::dir_ls(test[i]))
            ))
paste("fd")
!fs::file_exists(here(new_dir, basename(test[i]), basename(fs::dir_ls(test[i]))))



here(new_dir, basename(test[i]))
 paste0(new_dir, "/", basename(box[i]), "/")
```
