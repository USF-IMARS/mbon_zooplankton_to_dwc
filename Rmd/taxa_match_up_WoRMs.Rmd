...
# ---- load data set ----
file.taxa <- fs::dir_ls(path =  paste0(root,"/data/raw"),
           regexp = "^[^~].*\\.xlsx$")  %>%
    (function(fpath) {
...
```{r match-data-taxa}
# ---- fix new taxa names ----
# TODO: read in all files, joing with scientific names, then add more if they 
# don't exist
...
if (nrow(taxa_to_match_merg) > 0) {
    ...
    taxa_mtchnew <- bind_cols(taxa_to_match_merg, taxa_new)
    taxa_matched <-
        bind_rows(taxa_matched, taxa_mtchnew)
