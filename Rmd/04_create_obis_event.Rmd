...
if (!exists("aphia_id")) {
  aphia_id <-
    fs::dir_ls(
      path = here::here("data", "metadata", "aphia_id"),
      regexp = "^[^~]*(aphia)+.*\\.csv$"
    ) %>%
    last_mod(.) %>%
...
taxa_original <-
  ...
  mutate(
    ...
    maximumDepthInMeters = case_when(
      str_detect(site, "MR") ~ max_depth$MR,
      str_detect(site, "LK") ~ max_depth$LK,
      str_detect(site, "WS") ~ max_depth$WS,
      str_detect(site, "57") ~ max_depth$`57`,
      str_detect(site, "54") ~ max_depth$`54`,
      str_detect(site, "9B") ~ max_depth$`9B`
    ),
...
# ============================================================================ #    
  mutate(
    ...
    lifeStage,
    occurrenceStatus     = "present",
    preparations         = str_c("10% formalin before analysis",
...
# ============================================================================ #
# ---- 2.2. Measurement or Fact Additional Terms ----
# ============================================================================ #    
  mutate(
    Net_Type   = "bongo nets",
...
### 2.3.3 Measurement or Fact Core (MoF)
#### 2.3.3.1 MoF Base Info
...
test %>%
    ...
    select(-c(aphiaID:identifiedByID)) %>%
    pivot_longer(
                 cols = MoF1$orig_term,
                 names_to = "orig_term",
...
test %>%
    distinct(eventID, .keep_all = TRUE) %>%
    select(eventID,
          filter(MoF1, str_detect(event_occur, "event"))$orig_term
           ) %>%
     pivot_longer(
                 cols = -eventID,
                 names_to = "orig_term",
...
test %>%
    ...
    select(eventID, occurrenceID,
          filter(MoF1, str_detect(event_occur, "occur"))$orig_term
           ) %>%
     pivot_longer(
    ...
    left_join(MoF1, by = "orig_term") %T>% print()
...
left_join(event, location) %>%
    select(-cruise_id, -parentEventID, -rowNumber, -mesh) %>%
    # group_by(eventID) %>%
...
#                         "{cruise_id}:stn{locationID}:{mesh}um:{date}"
#         )  %>% str_remove("\\+0000"),
#         fieldNumber        = glue("{cruise_id}-{station}-{mesh}"),
...
#                "number_ind_sample" = organismQuantity, 
#                # "ind_m3" = organismQuantity,
#                verbatimIdentification, scientificName) %>%
#         
#         pivot_longer(
#                      # cols = c(number_ind_sample, ind_m3),
#                      cols = c(number_ind_sample),
#             # cols = c("split_amount", "mesh", "ind_m3"),
#                      names_to = "orig_term",
#                      values_to = "measurementValue",
#             values_transform = list(measurementValue = as.character)
#         ) %>%
#         select(occurrenceID, eventID, orig_term, measurementValue) %>%
#         left_join(MoF1, by = c("orig_term")) 
#         filter(event_occur == "event")
# 
# MoF <- 
