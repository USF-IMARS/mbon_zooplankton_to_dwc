...
taxa_data <-
    file.taxa %>%
    mutate(
    data = map(files, ~ load_data(.x, verbose = FALSE), .id = "files")
    ) %>%
    unnest(data)

head(taxa_data)
distinct(taxa_data, cruise_id)
```
...
# taxa_unmatch(taxa, lifestage, .file_expr = file_exprs, viewer = TRUE)
```
...
here("data", "processed", "ind_file_merg") %>%
    dir_ls() %>%
    file_copy(., here(cloud_dir, "processed", "ind_file_merg", Sys.Date()))
