...
taxa_info %>%
    ...
    distinct()
lvl <- names(infos) %>%
    fct(levels = c("search_name", "scientificname", 
                   "Species",
                   "Genus", "genus",
                   "Family", "family",
                   "Order", "order",
...
infos %>%
    mutate(
        .before = 1,
        flag = case_when(
            !str_equal(Genus, genus) ~ 6,
            !str_equal(Family, family) ~ 5,
            !str_equal(Order, order) ~ 4,
            !str_equal(Class, class) ~ 3,
            !str_equal(Phylum, phylum) ~ 2,
            !str_equal(Kingdom, kingdom) ~ 1,
            .default = NA
        )
    ) %>%
      arrange(desc(flag)) %>%
