...
TODO: need to read old meta_combined and add to the end instead of recreating 
every time
...
# select location where metadata is stored
# default: cloud 
if (!exists("main_cloud_dir")) main_cloud_dir <- rstudioapi::selectDirectory()

meta_prev_full <- here(cloud_dir, "cruise_logsheets") %>%
  dir_ls(regexp = "meta_combined.csv") %>%
  print() %>%
  read_csv(show_col_types = FALSE) 

meta_prev <-meta_prev_full %>%
  distinct(cruise_id) %>%
  pull(cruise_id) 

message(glue("Previous cruises: ", paste(meta_prev, collapse = ", ")))

# add additional ship acronyms to search 
ship_acronym <- "(WS|SV|HG|H|WB)\\d{4,5}"

# select all cruise metadata files
metadata_paths <-
  main_cloud_dir %>%
  dir_ls(type = "directory") %>%
  dir_ls(regexp = "metadata", recurse = 1) %>%
  str_subset(paste(c(meta_prev, "FK"), collapse = "|"), negate = TRUE) %>%
...
meta_skips <-
  ...
  mutate(
    base      = basename(file),
    cruise_id = str_extract(file, ship_acronym),
    sheets =
...
meta <-
  meta_skips %>%
    # filter(str_detect(cruise_id, "WS16319")) %>%
      ...
      function(.x, .y, .z, .c) {
        ...
        temp <-
          readxl::read_xlsx(
            ...
            na = c("skip", "Did not collect", "not recorded", "na", "Did Not Collect")
          ) %>%
        ...
        if (!str_detect(.c, "WS16319|HG26137")) {
          temp <- temp %>%

          # drop rows when flowmeter_out = NA
          drop_na(flowmeter_out)
        
          # filter(!(is.na(flowmeter_out) & is.na(date))) 
        }
        temp <- temp %>%
          mutate(
            across(
        ...
        # convert lat_in/lon_in from deg minutes to decimal degrees if class of
        # is of type string
        ...
        if (any(str_detect(colnames(temp), "notes"))) {
            temp <- mutate(temp, notes = as.character(notes))
        }
...
*NOTE*: WB22215 does have samples, just no flowmeter
...
meta_merg <- 
  meta %>%
    # filter(str_detect(base, "WS25221")) %>%
...
meta %>%
  filter(str_detect(cruise_id, "HG26137")) %>%
  pull(data)
...
meta_fix <-
  ...
  mutate(
    ...
    ship_speed_knots = hablar::retype(ship_speed_knots),
...
meta_calc <-
  meta_fix %>%
  # separate 200 and 500 mesh into separate rows
  separate_rows(mesh_size_um, sep = "/") %>%
  mutate(
    mesh_size_um     = str_trim(mesh_size_um),
    mesh_size_um     = retype(mesh_size_um),
  ...
  mutate(
    # Note: Sea Gear MF315 flowmeter correction
    # this correction is needed because flowmeter readings should have a tenths
    # place 
     flowmeter_in = case_when(
      str_detect(cruise_id, "WS18008") ~ flowmeter_in,
      inpeller_constant == 0.245  & date_time < ym("2025-06") ~ flowmeter_in / 10,
      .default = flowmeter_in / 10
    ),
     flowmeter_out = case_when(
      str_detect(cruise_id, "WS18008") ~ flowmeter_out,
      inpeller_constant == 0.245 & date_time < ym("2025-06") ~ flowmeter_out / 10,
      .default = flowmeter_out / 10
    ),
    inpeller_constant = if_else(
      inpeller_constant < 0.1,
      inpeller_constant * 10,
      inpeller_constant
      ),
    ...
    # add notes about questionable volume
    vol_notes = case_when(
      volume_filt_cubic_m < 0 ~ "volume filtered unusable, volume negative",
      volume_filt_cubic_m <= 10 ~ "volume filtered questionable, volume lower than expected",
      volume_filt_cubic_m > 100 ~ "volume filtered questionable, volume higher than expected",
      is.na(volume_filt_cubic_m) ~ "volume not recorded",
      .default = NA
    )
  ) %>%
...
volume filtered < 0 or > 100 

```{r errors}
meta_calc %>%
  filter(volume_filt_cubic_m > 100 | volume_filt_cubic_m <= 0) %>%
  select(
    -c(file, net_size:tow_speed_m_sec, ship_speed_knots, inpeller_constant, 
       split_size, contains("ignore"))
    ) %>%
  arrange(volume_filt_cubic_m) %>%
  mutate(volume_filt_cubic_m = round(volume_filt_cubic_m, 2))

meta_calc %>%
  filter(volume_filt_cubic_m > 100 | volume_filt_cubic_m <= 5) %>%
  select(
    -c(file, net_size:tow_speed_m_sec, ship_speed_knots, inpeller_constant, 
       split_size, contains("ignore"))
    ) %>%
  arrange(volume_filt_cubic_m) %>%
  mutate(volume_filt_cubic_m = round(volume_filt_cubic_m, 2)) %T>%
  {
    x <- count(., cruise_id) %>%
    print()
    
    meta_calc %>%
    filter(cruise_id %in% pull(x, cruise_id)) %>%
    print()
  ...
  {
    x <- count(., cruise_id, station, mesh_size_um) %>%
    print()
    print(1)
    meta_calc %>%
    filter(
      cruise_id %in% pull(x, cruise_id)
      & station %in% pull(x, station)
      & mesh_size_um %in% pull(x, mesh_size_um)
      ) %>%
    print()
...
```{r plots}
vol_fil_expr <- expression("Volume filtered ("*m^3*")")

meta_calc %>% 
  filter(str_detect(locationID, "MR|LK|WS") & mesh_size_um != 100) %>%
  ...
  labs(title = "All" , x = NULL, color = "Site",
       y = vol_fil_expr) +
    ggthemes::theme_calc()
...
#   labs(title = "Limit -100 - 1200 m^3", x = NULL, , color = "Site",
#        y = vol_fil_expr) +
#     ggthemes::theme_calc()

meta_calc %>% 
  filter(str_detect(locationID, "MR|LK|WS") & mesh_size_um != 100)  %>%
  filter(volume_filt_cubic_m > 5 & volume_filt_cubic_m < 100) %>%
  ggplot(aes(x = date, y = volume_filt_cubic_m, color = locationID)) +
  geom_point(na.rm = TRUE) +
  facet_grid(cols = vars(locationID), rows = vars(mesh_size_um), scales = "free") +
  scale_y_continuous(limits = c(0, 105), expand = expansion()) +
  labs(title = "Limit 5 - 100 m^3", x = NULL, , color = "Site",
       y = vol_fil_expr) +
    ggthemes::theme_calc()
    
# volume filt <= 0 or > 100   
meta_calc %>%
  filter(str_detect(locationID, "MR|LK|WS") & mesh_size_um != 100) %>%
  filter(volume_filt_cubic_m <= 5 | volume_filt_cubic_m > 100) %T>%
  print() %>%
  ggplot(aes(x = date, y = volume_filt_cubic_m, color = locationID)) +
  geom_point(na.rm = TRUE) +
  facet_grid(cols = vars(locationID), rows = vars(mesh_size_um), scales = "free") +
  labs(title = "Limit <5 and >100 m^3", x = NULL, , color = "Site",
       y = vol_fil_expr) +
  ggthemes::theme_calc() +
...
meta_calc %>% 
  ...
  labs(title = "Time Tow vs  Vol", color = "Year", 
       x = "Time Towed (min)",
       y = vol_fil_expr) +
  ggthemes::theme_calc()
...
meta_calc %>% 
  ...
  geom_point(na.rm = TRUE) +
  facet_grid(cols = vars(locationID), rows = vars(mesh_size_um), scales = "free") +
  scale_y_continuous(limits = c(-10, 100), expand = expansion(0, c(0, 100)),
                     oob = scales::oob_squish) +
  scale_x_continuous(limits = c(0, NA), expand = expansion(0, c(0, 1)), 
                     breaks = seq(0, 8)) +
  labs(title = "Time Tow vs  Vol Limit -10 to 100", color = "Year", 
       x = "Time Towed (min)",
       y = vol_fil_expr) +
  ggthemes::theme_calc()
...
meta_calc %>% 
  ...
  geom_point(na.rm = TRUE) +
  facet_grid(cols = vars(locationID), rows = vars(mesh_size_um), scales = "free") +
  scale_y_continuous(limits = c(-10, 105), expand = expansion(0, c(0, 10)),
                     oob = scales::oob_squish) +
  scale_x_continuous(limits = c(0, NA), expand = expansion(0, c(0, 1)), 
                     breaks = seq(0, 8)) +
  labs(title = "Time Tow vs  Vol Limit -10 to 105", color = "Year", 
       x = "Time Towed (min)",
       y = vol_fil_expr) +
  ggthemes::theme_calc()
...
meta_calc %>%
  filter(str_detect(locationID, "MR|LK|WS") & mesh_size_um != 100) %>%  
  print() %>%
  ggplot(aes(x = date, y = flowmeter_diff, color = locationID)) +
  geom_point(na.rm = TRUE) +
  facet_grid(cols = vars(locationID), rows = vars(mesh_size_um), scales = "free") +
  labs(title = "Flowmeter Diff", x = NULL, color = "Site",
       y = vol_fil_expr) +
  ggthemes::theme_calc() +
...
meta_final <-
  ...
  bind_rows(
    mutate(meta_prev_full, date = as_date(date, format = "%m/%d/%Y")),
    .
  ) %>%
...
if (FALSE) {
  ...
  meta_final %>%
    ...
    relocate(contains("notes"), .after = volume_filt_cubic_m) %>%
...
vol_filt_speeds <- 
  ...
  mutate(
    ...
    distance_m,
    ship_speed_knots,
    ship_speed_m_s = ship_speed_knots * 0.514444,
    tow_time_sec = tow_time_min * 60,
    tow_dist = ship_speed_m_s * tow_time_sec,
    tow_dist_1kt = 0.514444 * tow_time_sec,
    tow_dist_2kt = 0.514444 * 2 * tow_time_sec,
    volume_filt_cubic_m,
    vol_filt_time = net_area * tow_dist,
    vol_filt_time_1kt = net_area * tow_dist_1kt,
    vol_filt_time_2kt = net_area * tow_dist_2kt,
    check_out_flow = case_when(
      between(volume_filt_cubic_m, vol_filt_time_1kt, vol_filt_time_2kt) ~ "within range",
      volume_filt_cubic_m < vol_filt_time_1kt ~ "low",
      volume_filt_cubic_m > vol_filt_time_2kt ~ "high",
    ),
    check_out_time = case_when(
      between(vol_filt_time, vol_filt_time_1kt, vol_filt_time_2kt) ~ "within range",
      vol_filt_time < vol_filt_time_1kt ~ "low",
      vol_filt_time < vol_filt_time_2kt ~ "high",
    ),
    diff_flow_time = volume_filt_cubic_m - vol_filt_time
  ) %>%
  relocate(volume_filt_cubic_m, .before = vol_filt_time)

vol_filt_speeds

if (FALSE) {
  file_path_speeds <- rstudioapi::selectDirectory()
  write_csv(
    vol_filt_speeds,
    here(file_path_speeds, "sfer_mbon_vol_filt_speeds.csv")
  )
}
...
# NOAA Written Logs Digitized Zooplankton Net Tows from 2015 - March 2017
...
noaa_zoo_metadata <-
  ...
  mutate(
    # Note: flowmeter is MF315, assume reading in tenths so need to divide by 10
    # WS18008 is not included so no issues with dividing by 10
    flowmeter_in = flowmeter_in / 10,
    flowmeter_out = flowmeter_out / 10, 
    ...
    # add notes about questionable volume
    vol_notes = case_when(
      volume_filt_cubic_m <= 0 ~ "volume filtered unusable, volume negative or 0",
      volume_filt_cubic_m <= 10 ~ "volume filtered questionable, volume lower than expected",
      volume_filt_cubic_m > 100 ~ "volume filtered questionable, volume higher than expected",
      is.na(volume_filt_cubic_m) ~ "volume not recorded",
      .default = NA
...
noaa_zoo_metadata %>% 
  ...
  geom_point() +
  facet_grid(cols = vars(locationID), rows = vars(mesh_size_um), scales = "free") +
  scale_y_continuous(limits = c(-10, 700), expand = expansion())
    
# volume filt <= 0 or > 500   
noaa_zoo_metadata %>%
  filter(str_detect(locationID, "MR|LK|WS")) %>%
  filter(volume_filt_cubic_m <= 0 | volume_filt_cubic_m > 100) %T>%
  print() %>%
  ggplot(aes(x = date, y = volume_filt_cubic_m, color = locationID)) +
  geom_point() +
  facet_grid(cols = vars(locationID), rows = vars(mesh_size_um), scales = "free")  
...
noaa_zoo_metadata %>% 
  ...
  geom_point() +
  facet_grid(cols = vars(locationID), rows = vars(mesh_size_um), scales = "free") +
  scale_y_continuous(limits = c(-10, 1000), expand = expansion(0, c(0, 100))) +
  scale_x_continuous(limits = c(0, NA), expand = expansion(0, c(0, 1)), 
                     breaks = seq(0, 8)) 
...
# ============================================================================ #
# Merge IMaRS Logs with NOAA written logs
```{r}
# sheet from IMaRS
meta_df_imars <-
  # here(cloud_dir, "cruise_logsheets") %>%          # cloud
  here("data", "metadata", "cruise_logsheets") %>% # local
  dir_ls(regexp = "^[^~]*(meta_combined)+.*\\.csv$") %>%
  last_mod(.) %>%
  read_csv(show_col_types = FALSE) %>%
  # filter(date < as_date("2017-04-01")) %>%
  mutate(from = "imars", .before = 1) %T>% 
  print()
...
save_csv(
  .data          = meta_df,        
  save_location  = here("data", "metadata", "cruise_logsheets"),
  save_name      = "meta_combined_with_noaa",
  overwrite      = overwrite,
  verbose        = TRUE,
  time_stamp_fmt = "%Y%m%d_%H%M%S",
  utf_8          = FALSE
...
if (FALSE) {
  here("data", "metadata", "cruise_logsheets") %>%
  dir_ls(regexp = "meta_combined_with_noaa") %>%
  last_mod() %>% 
  file_copy(
      here(cloud_dir, "cruise_logsheets", "meta_combined_with_noaa.csv"),
      overwrite = TRUE
  )
}
```
