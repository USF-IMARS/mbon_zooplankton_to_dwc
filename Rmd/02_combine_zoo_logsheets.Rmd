...
meta_calc %>% 
  ...
  geom_point(na.rm = TRUE) +
  facet_grid(cols = vars(locationID), rows = vars(mesh_size_um), scales = "free") +
  scale_y_continuous(limits = c(-10, 105), expand = expansion()) +
  labs(title = "Limit -10 - 100 m^3", x = NULL, , color = "Site",
       y = vol_fil_expr) +
    ggthemes::theme_calc()
...
meta_calc %>%
  ...
  geom_point(na.rm = TRUE) +
  facet_grid(cols = vars(locationID), rows = vars(mesh_size_um), scales = "free") +
  labs(title = "Limit <0 and >500 m^3", x = NULL, , color = "Site",
       y = vol_fil_expr) +
  ggthemes::theme_calc() +
...
meta_calc %>% 
  ...
  scale_x_continuous(limits = c(0, NA), expand = expansion(0, c(0, 1)), 
                     breaks = seq(0, 8)) +
  labs(title = "Time Tow vs  Vol Limit -100 to 1000", color = "Year", 
       x = "Time Towed (min)",
       y = vol_fil_expr) +
  ggthemes::theme_calc()
...
meta_calc %>%
  filter(str_detect(locationID, "MR|LK|WS") & mesh_size_um != 100) %>%  print() %>%
  ggplot(aes(x = date, y = flowmeter_diff, color = locationID)) +
  geom_point(na.rm = TRUE) +
  facet_grid(cols = vars(locationID), rows = vars(mesh_size_um), scales = "free") +
  labs(title = "Flow Diff", x = NULL, color = "Site",
       y = vol_fil_expr) +
  ggthemes::theme_calc() +
