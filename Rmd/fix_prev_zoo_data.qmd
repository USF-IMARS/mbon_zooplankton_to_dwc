...
Step 1: *fix_prev_zoo*
Step 2: fix_prev_zoo_data_to_DwC

Inputs:  older raw data files from Jamie (excel spreadsheet)
Outputs: a `.csv` file with all data & metadata in one table.
...
Some sheets only had copepods counted
- list in the sheet `only conting copepods`
- LK03162002
- LK03165002
- MR03162002
- MR03165002
...
sheets <-
  ...
  mutate(
    year = str_c("20", year),
    other = case_when(
        str_detect(dup, "0") ~ "duplicate",
        str_detect(dup, "2") ~ "only copepods",
  ...
  mutate(
    ...
    meta = imap(
      ...
      \(.x, idx, paths) {
        ...
        dat <- dat %>%
            rename("Total.Ind..Sample" = `Total Ind. Sample` ) %>%
  ...
  mutate(
    calc =
      case_when(
        ...
        is.na(`D`) ~ "summation of individuals", 
...
test_calc %>%
  ...
  print() %T>%
  {
    filter(., str_detect(calc, "summation of individuals"))  %>%
    print()
  } %T>%
  {
    filter(., str_detect(calc, "correct"))  %>%
    print()
  } %T>%
  {
    filter(., str_detect(calc, "only number"))  %>%
    print()
  } %T>%
  {
    filter(., str_detect(calc, "no split size"))  %>%
    print()
  } 
...
species <-
  sheets %>%
  mutate(
    # extract metadata (i.e first 7 columns)
    meta = imap(
      sheet_nm,
      \(.x, idx, paths) {
        print(paste(idx, .x))
    ...
    meta_top = map(
      ...
      function(x) {
        ...
        if (any(str_detect(x$x, "^\\.$"), na.rm = TRUE)) {
...
species_unnest <-
  ...
  mutate(
    ...
    # this vol_sample_water is dillution factor
...
```{r}
species_unnest %>%
  select(sheet_nm, stn, mesh,
         vol_filtered_m3:counted_aliquot,
         clasification:total_ind_correct) %>%
  mutate(
    .before = total_ind_sample,
    ind_count2 = case_when(
        !is.na(no_ind_aliquot) ~ no_ind_aliquot * counted_aliquot,
        .default = ind_count
    ),
    diff = ind_count - ind_count2,
    divs = ind_count2 / ind_count,
    divs = round(divs, 0)
  ) %T>% 
  print() %>%
  filter(abs(diff) > 0) %>%
...
# created in 02_combined_zoo_logsheets to include both missing written logs from
# NOAA and logs written by IMaRS
meta_df <- 
  here("data", "metadata", "cruise_logsheets") %>%
...
missing two cruise: 
ok - 2016-11 LK 64 - because no record out volume, had MR here originally
...
```{r}
# March 2016 - duplicate issue
species_unnest %>%
  nest(.by = sheet_nm:meta, .key = "nested_data") %>%
  # filter out all 0316 samples
  filter((year == 2016 & month == 3))
  # filter(
...
species_merged_meta <-
  species_unnest %>%
  nest(.by = sheet_nm:meta, .key = "nested_data") %>%
  # filter out 0316 March 2016 - duplicate issue
  filter(!(year == 2016 & month == 3)) %T>% 
  print() %>%
  left_join(
  # full_join(
    nest(meta_df, .by = c(1:day, date, time_gmt), .key = "values"),
    by = join_by(
      year,
      month,
      "stn" == "locationID",
...
species_merged_filt <- 
  ...
  unnest(nested_data) %>%
  janitor::remove_empty("cols") %T>% 
  print()
```

# Set Duplicates and Missing Data
```{r}
get_dupes(species_merged_meta, sheet_nm:nested_data)
get_dupes(species_merged_filt)
visdat::vis_miss(species_merged_filt)

num_rows <- nrow(species_merged_filt)
num_col <- ncol(species_merged_filt)
visdat::vis_miss(slice(species_merged_filt, 1:(num_rows / 4)))
# visdat::vis_miss(slice(species_merged_filt, 1:(num_rows / 4))[,1:((num_col)/2)])
# visdat::vis_miss(slice(species_merged_filt, 1:(num_rows / 4))[,((num_col-1)/2):num_col])
visdat::vis_miss(slice(species_merged_filt, (num_rows / 4):(num_rows / 4 * 2)))
visdat::vis_miss(slice(species_merged_filt, (num_rows / 4 * 2):(num_rows / 4 * 3)))
visdat::vis_miss(slice(species_merged_filt, (num_rows / 4 * 3):(num_rows)))

visdat::vis_miss(species_merged_filt[,1:((num_col) / 2)])
visdat::vis_miss(species_merged_filt[,floor((num_col - 1) / 2):num_col])
...
species_merged_filt %>%
  ...
  select(
    -meta, -vol_filtered_m3, -total_ind_sample, -n_ind_m3,
    -contains("ignore"),
    ) %>%
  save_csv(
    .data          = .,
    save_location  = here(base_dir, "processed"),
    save_name      = "zoo_compiled_with_meta_essentials",
    # overwrite      = TRUE, # comment in when overwriting
    verbose        = TRUE,
    time_stamp_fmt = "%Y%m%d"
)
...
if (FALSE) {
  ...
  last_mod() %>%
  file_copy(
    here(cloud_dir, "old_jaime_data", "zoo_compiled_with_meta.csv"),
    overwrite = TRUE
    )
}
...
species_merged_filt %>%
  summarise(
    ...
    new_ind = sum(n_ind_m3, na.rm = TRUE)
...
species_merged_filt2 %>%
  ...
  ggplot(aes(x = date_time)) +
    ...
    geom_line(
      data = new_cal, aes(y = new_ind), color = "red") +
    # geom_point(aes(y = old_ind), color = "red") +
    geom_point(aes(y = new_ind), color = "blue") +
...
```{r}
new_filt <- distinct(new_data, cruise_id, "stn" = site, date, date_time, 
           mesh, volume_filt_cubic_m) 

test <- species_merged_filt %>%
  distinct(sheet_nm, year, month, stn, mesh, vol_filtered_m3, date_time) 

test %>%
  ggplot(aes(date_time)) +
  geom_line(aes(y = vol_filtered_m3), color = "blue") +
  geom_point(aes(y = vol_filtered_m3), color = "blue") +
  geom_line(data = new_filt, aes(x = date_time, y = volume_filt_cubic_m), color = "red") +
  geom_point(data = new_filt, aes(x = date_time, y = volume_filt_cubic_m), color = "red") +
  # coord_cartesian(ylim = c(-5, NA)) +
  coord_cartesian(ylim = c(-5, 1200)) +
  theme_minimal() +
  facet_wrap(stn ~ mesh, ncol = 3, scale = "free_y")
