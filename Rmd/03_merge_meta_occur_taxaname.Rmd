...
meta_df <-
  ...
  # filter date for when Natalia LF started analysis
  filter(date >= date("2017-06-18")) %>%
    
  # add max depth
  left_join(
    other_vars$max_depth, 
    by = c("locationID" = "site")
    ) %>%
  relocate(
...
taxa_files <-
  dir_ls(
    path = here("data", "processed", "all_merged"),
...
distinct(meta_df, cruise_id)
distinct(meta_df, cruise_id, mesh_size_um)
distinct(meta_df, cruise_id, station, mesh_size_um)
```
...
taxa_original <-
  ...
  # relocate(contains(c("ignore", "duplicate")), .after = last_col()) %>%
  select(-contains("ignore")) %T>% 
  print()
```
...
visdat::vis_miss(taxa_original[,1:20])
visdat::vis_miss(taxa_original[,21:40])
visdat::vis_miss(taxa_original[,41:60])
visdat::vis_miss(taxa_original[,61:ncol(taxa_original)])
```
...
# not sure what files were supposed to be used in `meta` and `dat`
```{r}
meta <- 
