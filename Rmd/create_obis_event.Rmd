...
occurance = institution_code = "USF_IMaRS" collection_code =
"compiled_zoo_taxonomy_nlf" catalog_number = "20yy_mm_dd" occurrenceID =
(unique identifier) joining of \* <urn:catalog>, \* institution code \*
collection_code \* catalog_number \* row number \* join by : eventDate =
date_time decimalLongitude = lon_in decimalLatitude = lat_in
scientificName = natalia spreadsheet scientificNameID = "TODO: worms
lookup", #taxonomy_df["classification"],

occurence = basisOfRecord = "HumanObservation", collectionCode =
collection_code, catalogNumber = catalog_number, occurrenceStatus =
"present", institutionCode = institution_code
...
meta_df <-
    ...
    mutate(
        distance_m_2        = (flowmeter_out - flowmeter_in) * inpeller,
        tow_speed_cm_sec    =  distance_m_2 * 100 / (tow_time_min * 60),
        volume_filt_cubic_m = net_area * distance_m_2,
        dist_dif            = distance_m_2 - distance_m,
        dist_speed          = tow_speed_cm_sec - tow_speed
        ) 
...
rcd_lvl <-
    ...
    dplyr::transmute(
        ...
        # recordedBy               = "Natalia LF?", # check these
...
df %>%
   dplyr::transmute(
       ...
       recordedBy               = "Natlia NF?", # check these
       recordedByID             = "may include orcid?", # check these
       ...
       lifeStage                = lifeStage, # need to inform Natalia this can be included
