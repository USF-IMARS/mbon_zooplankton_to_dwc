...
equation_zoo <- switch(
    which_one,
    "nat"    = expression(dillution * mean * 2 ^ (splits_analyzed)),
...
if (FALSE) {
   data.table::data.table(
    ...
    #     select(split_size) %>%
    #     mutate(
    #         # split_size2 = eval(parse(split_size))
    #         split_size2 = map(split_size, function(x) {
    #                         out <- tryCatch({
    #                             eval(parse(text = x))}, 
    #                             error = function(e) {NA_integer_})})  %>% 
    #                             unlist(.)
...
occur <-
    ...
    left_join(event, by = c("cruise_id", "site" = "locationID", "mesh")) %>%
    dplyr::transmute(
        scientificName, lifeStage, eventID,
