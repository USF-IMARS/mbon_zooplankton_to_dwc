...
# Load functions if not in .Rprofile
```{r load-functions}
# set to TRUE or T if you don't have .Rprofile setup in the project
# -or- for auto when restarting R
# copy paste: `usethis::edit_r_profile("project")`,
# then copy paste everything within the if statement 
if (FALSE) {
...
nlf = "Natlia Lopez Figueroa"
orcid_nat = "0000-0002-7527-0481" # TODO: add here
...
taxa_matched_merg <-
    ...
    mutate(
        ...
        # TODO: check splits_analyzed with natalia ----------------------------
        splits_analyzed = case_when(
            splits_analyzed == 0 | is.na(splits_analyzed) ~ 1,
            TRUE ~ splits_analyzed
        ),
        
        # TODO: check this equation -------------------------------------------
        number_ind_sample = dillution * mean * 2^(splits_analyzed),
        number_ind_sample = dillution * mean,
        number_ind_sample = dillution * mean, # the way jamie did it
        number_ind_sample = dillution * mean * splits_analyzed / pipette_vol_m_l, # the way jamie did it
...
rcd_lvl <-
    ...
    dplyr::transmute(
        ...
        recordedBy          = nlf, # check these
        recordedByID        = orcid_nat, # check these
    ) 
```
```{r event}
event <-
    ...
    dplyr::transmute(
        ...
        samplingEffor      = str_c(tow_time_min, "minutes", sep = " ")
    ) %>%
     distinct() %>%
     mutate(
...
occur <-
    ...
    dplyr::transmute(
        ...
        # individualCount          = "Number of Individuals per Sample",
        individualCount          = number_ind_sample,
        organismQuantity         = ind_m3,
        # organismQuantity         = "Individuals per cubic metre", # or `# Ind/Sample` and leave out individualCount?
