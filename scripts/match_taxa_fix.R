...
paged_worms_taxamatch_call <- function(page, 
    ...
    withRestarts(
        tryCatch({
            ...
            message(sprintf("Warning in %s: %s", 
                            deparse(w[["call"]]), w[["message"]]))
            
        }, error = function(e) {
            cli::cli_alert_danger(c("Couldn't find ", 
                                    "{.emph {cli::style_underline(page)}}\n"))
            if (.tryfix) invokeRestart("retry")
            
            return(NA)
        }),
        retry = function() {
            cli::cli_alert_warning("It might be mispelled!")
            new.name <- readline(
            ...
            if (new.name == "") {
                ...
                if (stringr::str_detect(r,"[yY]")) {
                    paged_worms_taxamatch_call(new.name, 
                                               .fuzzy = TRUE, 
                                               .taxa_type = "common", 
                                               .tryfix = TRUE)
                } else {
                    out <- paged_worms_taxamatch_call(new.name, 
                                               .fuzzy = TRUE, 
                                               .taxa_type = "taxa", 
                                               .tryfix = TRUE)
                }
...
# page = test_names
# test <- rep("", length(test_names))
# 
# test <- paged_worms_taxamatch_call(test_names[1], TRUE, .tryfix = TRUE)
# 
# stop()

match_taxa_fix <- function(names, ask = TRUE, fuzzy = TRUE,
                           taxa_type = "taxa") {
    ...
    results <- data.frame(scientificName   = character(), 
                          scientificNameID = character(), 
                          match_type       = character(), 
                          stringsAsFactors = FALSE)
    
    # main part of script  ------------------------------
    # initial attempt to find lsid from taxa name
    matches <- unlist(lapply(pages, paged_worms_taxamatch_call, 
                             .taxa_type = taxa_type, .fuzzy = fuzzy), 
                      recursive = FALSE)
    # matches2 <- lapply(pages, paged_worms_taxamatch_call, 
    #        .taxa_type = taxa_type, .fuzzy = fuzzy)
    
    # matches <-
        # purrr::map_dfr(pages, ~ paged_worms_taxamatch_call(.) ,
        #                      .taxa_type = taxa_type, .fuzzy = fuzzy, .id = "es") %>%
        # purrr::flatten_dfr(.id = "yes")
    
    # creates empty list if no taxa return name
    if (is.null(matches)) {
